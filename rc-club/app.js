...
function firebaseConfigured() {
  ...
  return values.length > 0
    && values.every((value) => value && !String(value).includes('PASTE_'))
    && adminEmail
    && !adminEmail.includes('PASTE_');
}
...
// Firestore does not permit an array directly inside another array. Race draws use
// nested arrays in memory, so store the race draw as JSON and restore it on read.
...
function renderResultEntry() {
  ...
  const positionOptions = (selected) => {
    ...
    for (let place = 1; place <= maxPlace; place += 1) {
      ...
      html += `<option value="${place}" ${Number(selected) === place ? 'selected' : ''}>${place}${suffix} — ${pts} pts</option>`;
    }
    return html;
  };
...
function renderAdminVisibility() {
  ...
  if (!isAdmin && $('admin').classList.contains('active')) setActiveTab('championship');
}
...
function renderMatch(match, roundIndex, matchIndex) {
  const { a, b } = match;
  if (!match.ready) return `<div class="race-card"><h4>Race ${matchIndex + 1}</h4><div class="driver-slot">TBD</div><div class="driver-slot">TBD</div></div>`;
  if (match.resolved && a && !b) return `<div class="race-card"><h4>Race ${matchIndex + 1}</h4><div class="driver-slot winner">${esc(driverName(a))} — BYE</div></div>`;
  if (match.resolved && !a && b) return `<div class="race-card"><h4>Race ${matchIndex + 1}</h4><div class="driver-slot winner">${esc(driverName(b))} — BYE</div></div>`;
  if (!a && !b) return `<div class="race-card"><h4>Race ${matchIndex + 1}</h4><div class="driver-slot">No race</div></div>`;
  if (match.resolved) {
    return `<div class="race-card"><h4>Race ${matchIndex + 1}</h4><div class="driver-slot ${match.winner === a ? 'winner' : ''}">${esc(driverName(a))}${match.winner === a ? ' ✓' : ''}</div><div class="driver-slot ${match.winner === b ? 'winner' : ''}">${esc(driverName(b))}${match.winner === b ? ' ✓' : ''}</div></div>`;
  }
  if (isAdmin) {
    return `<div class="race-card"><h4>Race ${matchIndex + 1} — choose winner</h4><button class="winner-button" data-knockout-winner="${roundIndex}|${matchIndex}|${esc(a)}">🏎️ ${esc(driverName(a))}</button><button class="winner-button" data-knockout-winner="${roundIndex}|${matchIndex}|${esc(b)}">🏎️ ${esc(driverName(b))}</button></div>`;
  }
...
function renderKnockout(race) {
  ...
  const bracket = rounds.map((round, roundIndex) => `<div class="round-column"><div class="bracket-round-title">${roundTitle(roundIndex, rounds.length)}</div><div class="bracket-round">${round.map((match, matchIndex) => renderMatch(match, roundIndex, matchIndex)).join('')}</div></div>`).join('');
  const finalMatch = rounds.at(-1)?.[0];
  const champion = finalMatch?.resolved && finalMatch?.winner ? `<div class="champion-banner">🏆 ${esc(driverName(finalMatch.winner))} wins the draw!</div>` : '';
...
function renderRace() {
  ...
  let content = '';
  if (race.type === 'knockout') content = renderKnockout(race);
  if (race.type === 'heats') {
    content = `<div class="race-grid">${race.groups.map((group, index) => `<div class="race-card"><h4>Heat ${index + 1}</h4>${group.map((id) => `<div class="driver-slot">${esc(driverName(id))}</div>`).join('')}</div>`).join('')}</div>`;
  }
  if (race.type === 'teams') {
  ...
  for (let roundIndex = 1; roundIndex < rounds.length; roundIndex += 1) {
    ...
    for (let matchIndex = 0; matchIndex < current.length; matchIndex += 1) {
      ...
      if (!left?.resolved || !right?.resolved) {
        Object.assign(match, { a: null, b: null, winner: null, ready: false, resolved: false });
        continue;
      }
      const oldA = match.a;
      const oldB = match.b;
      ...
      match.ready = true;
      if (!match.a && !match.b) Object.assign(match, { winner: null, resolved: true });
      else if (match.a && !match.b) Object.assign(match, { winner: match.a, resolved: true });
      else if (!match.a && match.b) Object.assign(match, { winner: match.b, resolved: true });
      else if (!(match.resolved && oldA === match.a && oldB === match.b && [match.a, match.b].includes(match.winner))) {
        match.winner = null;
...
function createKnockout(ids, seeded = false) {
  const ordered = seeded ? standings().filter((student) => ids.includes(student.id)).map((student) => student.id) : shuffle(ids);
  let size = 1;
  while (size < ordered.length) size *= 2;
  const padded = [...ordered, ...Array(size - ordered.length).fill(null)];
  const firstRound = [];
  if (seeded) {
    for (let i = 0; i < size / 2; i += 1) firstRound.push(makeFirstMatch(padded[i], padded[size - 1 - i]));
  } else {
...
function generateHeats(ids, heatSize, balanced) {
  ...
  const ordered = balanced ? standings().filter((student) => ids.includes(student.id)).map((student) => student.id) : shuffle(ids);
  if (!balanced) {
...
async function generateRace() {
  ...
  if (format.startsWith('knockout')) {
    ...
    race = { type: 'knockout', label: seeded ? 'Seeded Head-to-Head Knockout' : 'Random Head-to-Head Knockout', week, present, createdAt: new Date().toISOString(), rounds: createKnockout(present, seeded) };
  } else if (format.startsWith('heats')) {
    const balanced = format === 'heats-balanced';
    race = { type: 'heats', label: balanced ? 'Balanced Heats' : 'Random Heats', week, present, createdAt: new Date().toISOString(), groups: generateHeats(present, Number(els.heatSize.value || 4), balanced) };
  } else {
    race = { type: 'teams', label: 'Team Relay — Teams of 3', week, present, createdAt: new Date().toISOString(), groups: generateTeams(present) };
  }

  const previousRace = data.currentRace;
  data.currentRace = race;
  const saved = await saveData('Race draw generated.');
  if (!saved) {
    data.currentRace = previousRace;
    renderAll();
    return;
  }
...
async function clearRace() {
  ...
  const saved = await saveData('Race draw cleared.');
  if (!saved) {
    data.currentRace = previousRace;
    renderAll();
  }
...
async function saveRoundResults() {
  ...
  if (!week) return;
  const entries = [...document.querySelectorAll('[data-result-student]')].map((select) => ({ id: select.dataset.resultStudent, place: select.value ? Number(select.value) : null }));
...
async function signIn() {
  ...
  try {
    ...
    console.error(error);
    if (error?.code !== 'auth/popup-closed-by-user') showToast(`Google sign-in did not complete${error?.code ? ` (${error.code})` : ''}.`, true);
  }
}
...
function bindEvents() {
  ...
  els.adminButton.addEventListener('click', () => isAdmin ? setActiveTab('admin') : els.loginModal.classList.remove('hidden'));
  ...
  els.raceFormat.addEventListener('change', () => els.heatSizeWrap.classList.toggle('hidden', !els.raceFormat.value.startsWith('heats')));
